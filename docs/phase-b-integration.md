...
**Status:** ✅ **shipped & verified end-to-end 2026-07-18.** Both sides (portal + scorecard) are
deployed and the full ring is live: a stage change in Attio propagates to `channel_deals` and the
portal `deals`, and back. This is the shared contract between the **Deals Portal** repo
...
2. ✅ Portal: build `deal-sync-inbound` (write-if-changed, `X-Sync-Secret`, email suppression by construction). Deployed to `hkpglfdslglrjcgzbqpx`; `SYNC_SECRET` set.
...
5. ✅ Verified end-to-end: an Attio stage change on a portal-originated deal propagated to `channel_deals` and the portal `deals` row.
